...
bearerToken = os.environ.get('GITHUB_TOKEN')
...
def getIssuesPagedRequest(owner, name, cursor):
    ...
    result = requests.post('https://api.github.com/graphql', json={
                           'query': issueQuery, 'variables': variables}, headers=getHeaders()).json()
    return result
...
def getReactionsForCommentsRequest(owner, name, issueNumber, cursor):
    ...
    result = requests.post('https://api.github.com/graphql', json={
                           'query': query, 'variables': variables}, headers=getHeaders()).json()
    return result
